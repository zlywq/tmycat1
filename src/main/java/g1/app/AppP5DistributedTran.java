...
public class AppP5DistributedTran {
    ...
    public void doWholeProcessSimplyWhenFirst(StructIdsTransfer idsObj, boolean needInit, long transferAmountYuE, long transferAmountJiFen){
        ...
        if (needInit){
            initUserAccount(fromUserId,toUserId,InitAmount_YuE,InitAmount_JiFen);
        }
        ...
        try {
            doWholeProcessSimplyWhenFirst_step1createTransferRequest(idsObj.atrId, fromUserId, toUserId, transferAmountYuE, transferAmountJiFen);
            doWholeProcessSimplyWhenFirst_step2TransferJiFenAtFromSide(idsObj.atrId,fromUserId,transferAmountJiFen);
            doWholeProcessSimplyWhenFirst_step3TransferYuEAtFromSide(idsObj.atrId,fromUserId,transferAmountYuE);
        }catch (Exception e){
        ...
        try {
            doWholeProcessSimplyWhenFirst_step4TransferJiFenAtToSide(idsObj.atrId,toUserId,transferAmountJiFen);
            doWholeProcessSimplyWhenFirst_step5TransferYuEAtToSide(idsObj.atrId,toUserId,transferAmountYuE);
            doWholeProcessSimplyWhenFirst_step6updateRequestState(idsObj.atrId);
        }catch (Exception e){
            //不需回滚，只需重试直到成功
    ...
    public void checkAnddoWholeProcess(long atrId){
        ...
        try {
            ...
            try {
                doWholeProcessSimplyWhenFirst_step2TransferJiFenAtFromSide(atrObj.getAtrId(),atrObj.getFromUserId(),transferAmountJiFen);
                doWholeProcessSimplyWhenFirst_step3TransferYuEAtFromSide(atrObj.getAtrId(),atrObj.getFromUserId(),transferAmountYuE);
            }catch (Exception e){
                //需要回滚
                rollbackWholeProcess(atrObj.getAtrId());
                return;
            ...
            try {
                doWholeProcessSimplyWhenFirst_step4TransferJiFenAtToSide(atrObj.getAtrId(),atrObj.getToUserId(),transferAmountJiFen);
                doWholeProcessSimplyWhenFirst_step5TransferYuEAtToSide(atrObj.getAtrId(),atrObj.getToUserId(),transferAmountYuE);
                doWholeProcessSimplyWhenFirst_step6updateRequestState(atrObj.getAtrId());
            }catch (Exception e){
                //不需回滚，只需重试直到成功
    ...
    public void rollbackWholeProcess(long atrId){
        ...
        try {
            ...
            boolean goOn = rollbackWholeProcess_step1updateStateToRollback(atrId);
            if (!goOn){
                return;
            }
            String nextCmd = rollbackWholeProcess_step2RollbackJiFenAtFromSide(atrId,atrObj.getFromUserId());
            if (!"next".equals(nextCmd)){
                //目前暂无别的情况
                return;
            }
            nextCmd = rollbackWholeProcess_step3RollbackYuEAtFromSide(atrId,atrObj.getFromUserId());
            if (!"next".equals(nextCmd)){
                //目前暂无别的情况
                return;
            }
            nextCmd = rollbackWholeProcess_step4RollbackJiFenAtToSide(atrId,atrObj.getToUserId());
            if (!"next".equals(nextCmd)){
                //目前暂无别的情况
                return;
            }
            nextCmd = rollbackWholeProcess_step5RollbackYuEAtToSide(atrId,atrObj.getToUserId());
            if (!"next".equals(nextCmd)){
                //目前暂无别的情况
                return;
            }
            rollbackWholeProcess_step6updateRequestState(atrId);
        }catch (Exception e){
            //过程中有异常，需要重试，可以根据具体异常立即重试或者延时重试......
        }finally {
            //释放分布式锁
